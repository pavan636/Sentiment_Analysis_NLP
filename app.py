...
def analyse():
    ...
    if request.method == 'POST':
        ...
        received_text=blob
        blob_sentiment,blob_subjectivity=blob_sentiment.polarity, blob.sentiment.subjectivity
        number_of_tokens = len(list(blob.words))
        nouns=list()
        for word, tag in blob.tags:
            if tag = 'NN':
                nouns.append(word.lematize())   # lematize is method to categorize every keyword 
